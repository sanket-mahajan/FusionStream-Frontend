...
const Comments = () => {
  ...
  const { user } = useSelector((state) => state.auth);
  const { videoId, tweetId } = useParams();
  const eId = videoId || tweetId;
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 bg-gray-900 rounded-lg shadow-lg w-full text-white">
      {/* Add Comment Form */}
      <form onSubmit={handleSubmit} className="mb-6">
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            ref={inputRef}
            onChange={(e) => setContent(e.target.value)}
            className="flex-grow px-2 py-1 sm:px-4 sm:py-2 text-gray-800 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0"
            placeholder={
              parentId ? "Replying to a comment..." : "Add a comment..."
            }
          />
          <button
            type="submit"
            className="px-3 py-1 sm:px-6 sm:py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition-colors"
          >
            {parentId ? "Reply" : "Comment"}
          </button>
        </div>
      ...
      {comments &&
        comments.map((comment) => (
          <div
            key={comment._id}
            className="mb-4 p-2 sm:p-4 bg-gray-800 rounded-lg shadow-md"
          >
            {editCommentId === comment._id ? (
              <UpdateComment
                commentId={comment._id}
              ...
              <>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {user._id === comment.owner._id && (
                    <>
                      <button
                        onClick={() => setEditCommentId(comment._id)}
                        className="px-2 py-1 sm:px-4 sm:py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 text-xs sm:text-base"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="px-2 py-1 sm:px-4 sm:py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 text-xs sm:text-base"
                      >
                        Delete
                      </button>
                    </>
                  ...
                  <button
                    onClick={() => handleReply(comment._id)}
                    className="px-2 py-1 sm:px-4 sm:py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 text-xs sm:text-base"
                  >
                    Reply
                  </button>
                </div>
                ...
                {comment.replies &&
                  comment.replies.map((reply) => (
                    <div
                      key={reply._id}
                      className="mt-4 pl-4 sm:pl-6 border-l-2 border-gray-600"
                    >
                      <p className="mb-2">{reply.content}</p>
                      <div className="flex items-center mb-4">
                        <img
        ...
        <div className="mt-4 flex justify-center flex-wrap gap-1 sm:gap-2">
          {Array.from({ length: pagination.totalPages }, (_, index) => (
            <button
              key={index}
              onClick={() => handlePagination(index + 1)}
              className={`px-2 py-1 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-base ${
                currentPage === index + 1
                  ? "bg-blue-500 text-white"
                  : "bg-gray-700 hover:bg-gray-600"
              }`}
